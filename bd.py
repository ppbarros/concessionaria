...
def show_foto(cursor, idimg):
    cursor.execute(f'select image from teste where idteste={idimg}')
...
def show_vips(cursor):
    cursor.execute(f'select idcarros, marca, modelo, path_ft, preco from carros where vip = 1')
